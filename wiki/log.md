...
- 対応2（再発防止・PR、**マージ済み**）: 隙間は **mirror 側を広げて**埋める設計に。`WINDOW_DAYS` 既定 14→75日（canonicalの最大遅れ≈63日＋余裕）。canonical は無変更。README に盲点・サイズ根拠・月次run欠落時の注意を追記 → [digitaldemocracy2030/slack-logs PR #6](https://github.com/digitaldemocracy2030/slack-logs/pull/6) を squash merge。mirror を即時再実行し **live 反映を確認**（`mirror/sync.json` window_days=75、oldest=2026-06-07、59ch・2,294件）。canonical(〜06-30＋7/8月backfill)と連続し盲点解消
- 判明した既知の弱点（別対応）: mirror workflow の push retry が `git pull --rebase` を使うため、手動 dispatch と定期 run が重なるとバイナリ `.jsonl.gz` を rebase できず後発 run が失敗する。今回1件失敗→次の定期 run で自己修復・実害なし。auto起票された issue #7 は原因を添えて close 済み。手動実行は定期 run（`7 */6 * * *`）を避ける
- ドキュメント反映（完了）: [[アーカイブパイプライン設計]]（「二層の間に盲点を作らない」節を新設）・[[AI から Slack ログを参照するパターン]]・[[Slackログアーカイブ]] の「直近14日」を「直近75日」に更新し、盲点と75日の根拠を追記
- 未対応（別件）: OPEN issue `slack-backup failed: 2026-03` / `2025-07`（2026-06-09 起票）はブートストラップ時の失敗で今回の穴とは無関係
...
- 確認した制約:
  - canonical `raw/slack` に **2026-07・2026-08 のファイルは存在しない**（本文入りは 2026-06 が最後、1150件）。canonical は約2ヶ月遅延のため、7月分が入るのは9月下旬以降の見込み
  - mirror は rolling 14日のみ（現在 window 2026-08-06〜20）。前回読んだ 08-04〜18 とほぼ重なり、7月〜8月頭には届かない
  - 結論: **2026-07-01〜2026-08-05 はどの層にも本文がなく、現時点では読めない**
- 新規に読めた差分: 前回境界（08-18）以降の mirror 新着13件を通読。実質1スレッドのみ
  - サミットの[[ブロードリスニング]]解説セッションは録画が残らず未公開（前半=録画データなし／後半=音質不良）。書籍出版に合わせ別途録り直しYouTube公開する案が `2_broad-listening-book` で浮上（外部スタジオ収録を検討、2026-08下旬・未決）
- 更新: [[sources/digital-democracy-summit-2026-recordings|録画アーカイブ要約]]の概要に、BL解説が公開7本に含まれない理由と録り直し案を追記
- 次にできること（未実施）: 完全化した 2026-06 canonical（1150件）は従来ピンポイント参照のみで全ch面読みしていない。埋めるならここが最有力。7月分は9月下旬の canonical 反映を待つ

## [2026-08-19] ingest | Slack直近2週間（2026-08-04〜18）の通読と反映

- 元情報: `digitaldemocracy2030/slack-logs` mirror（window 2026-08-04〜18、59ch・164件）を全チャンネル通読
- 状況: 大きな話題（[[events/2026-08-02-digital-democracy-summit|サミット]]録画公開・[[topics/nl-data-collection-tools-collaboration|いどばた横断ドキュメント]]）は 08-17／08-18 の ingest で既反映済み。多くのチャンネルは新規メンバー1名の join 通知のみ
- 新規に反映した知見: ボードメンバー鈴木健が全体定例で「各プロジェクト（[[広聴AI]]・[[いどばた]]・[[Polimoney]]）の戦略会議」を提案（Google Meet・30〜60分・2026年9月以降、現状課題のボード支援と1年の方向性議論、広聴AIには自治体観点で関治之も招く方向）
- 更新: [[コミュニティ運営]]に「ボードによるプロジェクト戦略会議」節を追加
- 反映しなかったもの: 広聴AIアルゴリズム開発でのattention≒soft k-means談義（結論なしの思考メモ）、人数推移botが2026-07-25以降停止（transientな運用不具合）、各定例のリマインダbot投稿

## [2026-08-18] ingest | サミット2026 録画アーカイブ（7セッション）の字幕要約

- 元情報: コミュニティ運営担当（モアイ／小野）のSlack案内＋公開されたYouTube録画。音声確認のうえ配信可能な7セッションを公開（品川区セッションは非公開）
- プレイリスト: https://www.youtube.com/playlist?list=PLX1vATirF4MQ （公式「デジタル民主主義サミット2026」）。公開7本（政治資金／Generative Polis／オンオフ横断コミュニケーション／国会議員マップ／コアループ／源内／分かり合えなさの先へ）、非表示1本（品川区）
- 手法: yt-dlp で各動画の日本語自動生成字幕（ASR）を取得→整形→7本を並列要約。ASR起因の固有名詞・数値誤りは公式プログラム等で補正し、不確かなものは各節に明記
- 新規ページ: [[sources/digital-democracy-summit-2026-recordings|デジタル民主主義サミット2026 録画アーカイブ要約]]（7セッションの登壇者・経緯・論点・数値・dd2030との関連）
- 更新ページ: [[events/2026-08-02-digital-democracy-summit|デジタル民主主義サミット2026]] に「録画アーカイブの公開」節（公開7本の表＋要約ページへのリンク）を追加。frontmatter sources・公式情報・関連ページ・「今後の更新事項」を更新。[[sources/index]] にも登録
- 注意: 要約は逐語ではなくASRベース。引用・事実確認は元動画で行う前提

## [2026-08-18] file back | サミット2026 開催後のネット上の反応

- 元情報: Web検索で収集した、開催後の参加者・登壇者によるnote公開記事（Lily／さかいっこobasan／宮坂学／高木俊輔／モアイ小野の5本）。SNSの私的発言は含めず公開記事のみ
- 更新ページ: [[events/2026-08-02-digital-democracy-summit|デジタル民主主義サミット2026]] に「開催後のネット上の反応」節（投稿者別の反応表）を追加。frontmatter sources に note 5本を追加
- 分かったこと:
  - 反応はオードリー・タンの閉会メッセージ「閉じたループ／レシート」に集中。事実上のキーワードとして複数参加者に共有された
  - 宮坂学（東京都副知事）は鈴木健との対話を起点に「学習する都市（Learning City）＝民主主義は学習システム」を提起
  - 高木俊輔はLiquitous「過信と諦め、二つのデジタル民主主義幻想」に刺激を受け「執行の可視化」を提案。問題提起型セッションが二次考察を促した
  - モアイ（小野）はPolimoney政治資金セッションの補足で「透明性と見える化は別」と整理
- 未反映のリンク先: [[宮坂学]]・[[Liquitous]] はページ未作成（赤リンク）。Web検索はX個別投稿を拾いにくく、note中心の収集にとどまる

## [2026-08-18] file back | デジタル民主主義サミット 運営の記録（Slack）

- 元情報: Slack `dd_prance_event2026`（C0B4L1JB8RM、2026-05〜08、325件）と `7_デジタル民主主義サミット`（C0BME5MTWD8、30件）。digitaldemocracy2030/slack-logs の canonical + mirror を横断デコードして通読
- 追記先: [[events/2026-08-02-digital-democracy-summit|デジタル民主主義サミット2026]] に「## 運営の記録（企画・準備・振り返り）」節を新設
- 書き戻した内容（公式プログラム／閉会とは別レイヤーの舞台裏）:
  - 体制と役割分担（進行管理=尹世羅、総責任=鈴木健、企画=西田尚史、当日統括=スターン廉、ボランティア=nuun、録画=モアイ）
  - 主要な運営判断（3会場構成、Google Meet翻訳でのコスト圧縮、主催名義=デジタル民主主義2030、未確定登壇者のWeb非掲載、CFP比率）
  - 本番の技術トラブル（Meet録画が8時間で自動終了、マイク音量過大で音声途切れ）とアーカイブ選別（18本中7本公開）
  - 振り返りと来年への申し送り、開催後のAI「自己理解支援」＝誘導をめぐる議論
- 配慮: 進行管理者の離任は私的事情に踏み込まず「個人的事情により離任・引き継ぎ」に留めた（私的発言を持ち込まない規約）
...
- 新規ページ: [[topics/nl-data-collection-tools-collaboration|自然言語データ収集ツールの関係と「一本化しない」協働]]
- 保存した知見:
  - 広聴AI・いどばた・倍速会議・farbrain/divcon は「多くの人から自然言語データを集める」目的で共通だが、アーキが根本的に違い一本化すべきでない
  - Coding Agentでコード生産コストが激減 → 希少なのは「継続」と「参加」の価値 → プラグイン化で交換可能性を担保しつつ協働、という西尾の思想
  - embedding→クラスタリング（大規模向け）と long-context 全投入（小規模・理解しやすさ向け）の使い分け
  - いどばたの再整理（discon→リニューアル、ビジョン/政策の分離、放置された「いどばたボット」構想）、ぶるーも→kuboonの引き継ぎ不全に西尾が介入
- 更新: `wiki/index.md`（テーマ節に1行）

## [2026-08-03] ingest | サミット当日投影資料と国会議員マップ登壇全文を反映

- 確認: 主会場PDF 195ページ、第2会場PPTX 92スライド、第3会場PPTX 110スライドの全ページ一覧
- 新規ページ: [[sources/digital-democracy-summit-2026-materials|デジタル民主主義サミット2026 当日投影資料・登壇記録]]
- 外部記事: 国会議員マップ登壇全文 `https://note.com/kokkai_map/n/n602fbc87ad54`
- 追記: 国政ブロードリスニング事例、コアループの4段階と熟議結果、ブロードリスニングの技術・選択フロー、第2会場の制度実践、国会議員マップの開発経緯・中立性・運営課題
- ファイル方針: 投影資料3点は合計100MB超のためリポジトリへ複製せず、ユーザー提供のリポジトリ外ソースとしてファイル名と確認範囲を記録
- 更新: [[events/2026-08-02-digital-democracy-summit|デジタル民主主義サミット2026]]、[[sources/index|ソースカタログ]]から導線を追加

## [2026-08-03] events | オードリー・タンの閉会全文とSlido回答を反映

- 一次出典: `https://audreyt.org/dd`「ループを、閉じる」
- 確認: 18会場すべてのSlido質問266件に書面回答、未収録会場0、質問の省略0。43件は担当者フォローアップ、8件は参加者回答付き
- 追記: 「閉じたループ」と検証可能なレシートの4要素、ブロードリスニングの公共インフラとしての位置づけ、民主主義の学習速度、レースではなくミッションという整理
- 後続: 一年以内に日本の閉じたループをレシート付きで公開するという呼びかけを記録
- 修正: X投稿経由だった閉会内容の説明を、原典に基づく記述へ更新

## [2026-08-03] events | サミット閉会セッションと参加者投稿を追記

- 出典: 西尾泰和のX投稿 `https://x.com/nishio/status/2084132285042003982`、関連するSNS投稿のユーザー提供まとめ、オードリー・タン「Plurality & 6pack.care」
- 追記: AIで全セッションの文字起こしとSlido質問を横断した閉会回答、AIインタビューにおける本人性、発言前の利用目的と発言後のフィードバックという論点
- 事例: [[ブロードリスニング]]の技術セッションと、AIを使って政治を学びながら個人開発された「国会議員マップ」を整理
- 注意: SNS上の要約・感想は正式なセッション記録と区別し、アーカイブ公開後の再確認事項とした

## [2026-08-03] events | デジタル民主主義サミット2026を追加

- 新規ページ: [[events/2026-08-02-digital-democracy-summit|デジタル民主主義サミット2026]]
- 出典: 公式サイト `https://summit.dd2030.org/` と公式プログラム `https://summit.dd2030.org/program/`
- 整理した内容: 開催情報、共催体制、プログラムの主要テーマ、dd2030の各プロダクト・プロジェクトとの接続
- 注意: 8月3日時点では各セッションの実際の議論・結論、参加人数、録画・登壇資料の公開状況は未確認として明記
- 更新: [[index]]、[[overview]]、[[時系列まとめ]]から導線を追加

## [2026-08-03] events | デジタル民主主義サミット2026の開催後報告を追記

- 出典: 鈴木健による2026年8月3日のX投稿（nishioによるリポスト） `https://x.com/nishio/status/2084117942581731530`
- 追記: 全18セッションの実施、会場・オンライン双方からの参加、鈴木健が担当したセッション、アーカイブ配信予定
- 課題・後続: オンライン視聴の問題と改善意向、翌年開催の意向を主催者報告として記録
- 未確認: 具体的な参加人数、各セッションの議論・結論、アーカイブの公開URL
